...
<script>
import { requestAll as requestAllMenu } from '@/api/app/sys/menu'
  ...
  methods: {
    ...
    handleSetRole(row) {
      ...
      requestRoleMenuIDList(this.temp.id).then(response => {
        this.$refs.tree.setCheckedKeys(this.transIdToArray(response.data))
      }).catch(() => {
    ...
    setRole() {
      this.loading = true
      // 获取所有节点的id
      const menuids = this.$refs.tree.getHalfCheckedKeys().concat(this.$refs.tree.getCheckedKeys())
